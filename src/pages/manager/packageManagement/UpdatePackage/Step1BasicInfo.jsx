    ...
    useImperativeHandle(ref, () => ({
      submit: async () => {
        if (!currentTemplateId) {
          toast.error('Vui lòng chọn mẫu gói trước khi tiếp tục');
          return false;
        }
        return true;
      }
    ...
    return (
      <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <Typography variant="h6" gutterBottom sx={{ mb: 0.75, fontWeight: 600, fontSize: '1.1rem' }}>
          Bước {stepIndex + 1}/{totalSteps}: Cập nhật thông tin cơ bản
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1, fontSize: '0.875rem' }}>
          Kiểm tra hoặc thay đổi mẫu gói, tên và mô tả để phù hợp với chi nhánh.
        </Typography>
        ...
        <Box sx={{ mb: 2 }}>
          ...
          {!currentTemplateId && (
            <Alert severity="info" sx={{ mt: 1 }}>
              Vui lòng chọn mẫu gói để cập nhật các thông số gợi ý (giá, thời hạn, slot, lợi ích).
            </Alert>
          )}
        </Box>
...
Step1BasicInfo.displayName = 'UpdatePackageStep1BasicInfo';
