...
const Step2AssignRooms = forwardRef(({ data, updateData, stepIndex, totalSteps, roomOptions = [], dependenciesLoading = false, branchSlotId }, ref) => {
  const [selectedRoomIds, setSelectedRoomIds] = useState(() => {
    const ids = data.roomIds || [];
    return Array.isArray(ids) ? ids : [];
  });

  // Update selectedRoomIds when data.roomIds changes (from parent)
  useEffect(() => {
    const ids = data.roomIds || [];
  ...
  return (
    <Box>
      ...
      <FormControl fullWidth sx={{ mb: 2 }} disabled={dependenciesLoading || roomOptions.length === 0}>
